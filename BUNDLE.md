...
Без этого шага она поднимется, но останется молчаливой оболочкой: механика, память и
руки на месте, характера нет. Системный промпт собирается из `soul/SOUL.md` (ядро
персоны) и `soul/VOICE.md` (регистр, показанный примерами); `soul/skills/*.md` — ремесло,
которое подхватывается её же памятью как канон.

Этих файлов в репозитории нет намеренно. Характер — не пакет, который ставят из чужого
репозитория: он пишется тем, с кем этот компаньон будет жить, и под того, кем он будет.
Форма и то, что в них обычно бывает, — в `soul/SOUL.example.md` и `soul/VOICE.example.md`.

```bash
mkdir -p soul/skills && $EDITOR soul/SOUL.md
```

Файлы читаются при сборке каждого прохода, так что перезапуск не нужен — достаточно
следующего хода.
