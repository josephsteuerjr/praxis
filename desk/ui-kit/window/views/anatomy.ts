...
const INTRO: Array<[string, string]> = [
  ...
  [
    "Память",
    "Файлы, не база: события жизни, архивы комнат, дневник, досье людей, список желаний. Тул recall ищет по всему " +
...
const LAYERS: Array<[string, string]> = [
  ...
  ["runner.py — локальный код агента", "Слушает записки окна и Telegram и запускает ход агента (voice_turn_envelope из дерева). Своей логики хода у движка нет: только транспорт и конфиг."],
...
function fenceHTML(hands: HandFence[] | undefined, sandboxOn: boolean): string {
  ...
  return `<h3 class="section-title">Ограда поимённо <span class="muted">${inside.length} в ограде · ${outside.length} вне</span></h3>
    <div class="card">
      ...
      <p class="muted" style="margin-top:12px">Список считает сам код агента по живому набору тулов: накрыт тул оградой или нет, видно по тому, что на нём стоит, а не по имени в таблице. Тул, о котором ограда не знает, считается вне её.${sandboxOn ? "" : " Сейчас ограда выключена — поэтому вне её всё."}</p>
    </div>`;
}
