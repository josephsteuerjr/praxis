...
* сервер — канал рядом с деревом агента (``server/deploy_desk.py`` клал пять
...
не уехал в поставку) и «контейнер канала нечем пересобрать».
...
* ``desk.json`` — манифест: версия продукта, вид пакета, sha256 каждого файла и
  ...
  версия Praxis там живёт — оболочки, которая отвечает окну ``app_info``, там
  нет.
...
PARTS: tuple[Part, ...] = (
    ...
    # Praxis агент на сервере (тех карточек нет вовсе). До разделения фронт был
    ...
    Part("remote/dist", "static", (SERVER,), dist=True,
    ...
    Part("server/deskctl.py", "deskctl.py", (SERVER,), kind="file",
         why="надзор за контейнерами: они, их журналы и мозг — мимо агента"),
...
def product_version() -> tuple[str, dict]:
    ...
    declared = {
        ...
        # объявлений. Прибор читал восемь и сравнивал три: отставание `remote` или
        # `miniapp` проходило и его, и стенд — молча. Проверено приёмкой 11.09 откатом
        # версии в копии дерева: прибор не сказал ничего.
        "remote/package.json": _json_version(DESK / "remote" / "package.json"),
...
#: установки: настольный Praxis и Praxis на сервере — одно и то же окно, просто
#: одно живёт в exe, а другое отдаётся каналом по HTTP.
HELENE, REMOTE = "helene", "remote"
WINDOW_FRONT = {HELENE: "app/dist", REMOTE: "remote/dist"}


def copy_static(dest: Path, app: str = HELENE) -> str:
    """Статика окна ``app`` в ``dest`` + манифест рядом -> отпечаток.

    Отдельной ручкой, потому что настольное издание — это окно без канала: там
    ...
    безопасно. Теперь их две, и молчаливый `next(...)` уложил бы в Praxis окно
...
def build(dest: Path, flavor: str, *, version: str | None = None,
    ...
    for part in take:
        if part.dest == "static":
            ...
            # составе фронт Praxis, у Windows — фронт Элен.
            static_digest = copy_static(dest / "static",
                                        REMOTE if part.src == WINDOW_FRONT[REMOTE] else HELENE)
...
def verify(root: Path) -> list[str]:
    ...
    ⚠ На Windows установщик вправе ОСТАВИТЬ пользовательскую статику, если
    выпуск её не менял (`install.rs`, слово владельца 07.09). Тогда расхождение
    по `static/…` — не поломка, а решение установщика; всё остальное обязано
    сходиться.
    """
