...
/// Невидимый знак: Cc (управляющие, включая `\t` и U+0085), Cf (знаки формата —
/// мягкий перенос, ноль-ширины, переключатели направления письма), Zl (U+2028)
/// и Zp (U+2029).
///
/// ⚠⚠ ЗАЧЕМ ОТДЕЛЬНО ОТ `is_control()` (находка судей 19.09). Окно
/// подтверждения показывает владельцу команду и «зачем» — и решает он ПО ТОМУ,
/// ЧТО УВИДЕЛ. `is_control()` ловит только Cc; U+2028 и U+2029 — это переводы
/// строки для всего, что рисует текст, а U+200E/U+202E переставляют знаки
/// местами прямо на экране. То есть просьба могла нарисовать в окне владельца
/// одну команду, а уехать в исполнение другой — и тем же приёмом подделать
/// журнал, который читается построчно.
///
/// U+00A0 (неразрывный пробел) СЮДА НЕ ВХОДИТ намеренно: это обычный пробел
/// категории Zs, он видим, и отказывать из-за него — мешать по делу.
///
/// Список Cf — по таблице Unicode; std общих категорий не знает, а тянуть ради
/// одной проверки крейт с таблицами в три головы (окно, служба, мастер) дороже,
/// чем девять диапазонов под стендом.
#[allow(dead_code)]
fn broker_invisible(ch: char) -> bool {
    if ch.is_control() {
        return true; // Cc, включая \t, \n, \r и U+0085
    }
    matches!(ch,
        '\u{00ad}'                      // Cf: мягкий перенос
        | '\u{0600}'..='\u{0605}'
        | '\u{061c}'
        | '\u{06dd}' | '\u{070f}' | '\u{0890}' | '\u{0891}' | '\u{08e2}'
        | '\u{180e}'
        | '\u{200b}'..='\u{200f}'       // ноль-ширины, LRM/RLM
        | '\u{2028}'                    // Zl
        | '\u{2029}'                    // Zp
        | '\u{202a}'..='\u{202e}'       // переключатели направления письма
        | '\u{2060}'..='\u{2064}'
        | '\u{2066}'..='\u{206f}'
        | '\u{feff}'                    // BOM посреди строки
        | '\u{fff9}'..='\u{fffb}'
        | '\u{110bd}' | '\u{110cd}'
        | '\u{13430}'..='\u{1343f}'
        | '\u{1bca0}'..='\u{1bca3}'
...
/// Слова отказа — одни на обе стороны границы и на оба языка кода (здесь и в
/// `localharness/broker.py::INVISIBLE_SAID`).
#[allow(dead_code)]
const BROKER_INVISIBLE_SAID: &str =
    "в команде есть невидимые знаки переноса/формата (U+2028, U+200E, мягкий перенос и такие же): \
     окно подтверждения показало бы владельцу не то, что исполнится. Напиши команду обычными знаками";
...
impl BrokerAsk {
    ...
    fn parse(raw: &str) -> Result<BrokerAsk, String> {
        ...
        if why.chars().any(broker_invisible) {
            return Err(
                "«зачем» — одной строкой обычными знаками: журнал владельца читается построчно, \
                 и перевод строки (в том числе невидимый — U+2028, U+0085) подделывает в нём \
                 чужие записи, а знаки формата переставляют текст прямо на экране"
                    .into(),
            );
        }
        ...
        let args = match value.get("args") {
            ...
            Some(serde_json::Value::Array(items)) => {
                ...
                for item in items {
                    ...
                    // ⚠ ГРАНИЦА СДВИНУТА 19.09 (судьи). Раньше аргументу
                    // разрешался перевод строки «по делу», а подделку журнала
                    // снимал показ (`broker_shown_command`). Но решение владельца
                    // принимается ПО ТОМУ, ЧТО ОН УВИДЕЛ В ОКНЕ, и знак, которого
                    // на экране нет вовсе, там делать нечего ни в каком виде:
                    // многострочный скрипт отдают файлом, а не аргументом.
                    if text.chars().any(broker_invisible) {
...
fn broker_cmd_ok(cmd: &str) -> Result<(), String> {
    ...
    if cmd.contains('\0') {
        return Err("в имени программы нулевой байт — команда оборвётся на нём".into());
    }
...
fn broker_shown_command(cmd: &str, args: &[String]) -> String {
    // ⚠ Тот же набор, что на границе (`broker_invisible`), а не один
    // `is_control()`: строку читает ЧЕЛОВЕК, и U+2028 переносит её у всего, что
    // рисует текст, а U+202E разворачивает кусок задом наперёд прямо на экране.
    // Граница такие просьбы отвергает, но эта функция пишет ещё и журнал, и
    // подделать его она не должна ни при каких входных данных.
    let flat = |s: &str| -> String {
        s.chars().map(|c| if broker_invisible(c) { '·' } else { c }).collect()
    };
...
mod broker_protocol_tests {
    ...
    /// ⚠ Находка судей 19.09: окно подтверждения фильтровало только `is_control()`.
    /// U+2028 переносит строку у всего, что рисует текст, U+202E разворачивает
    /// кусок задом наперёд, мягкий перенос исчезает совсем — владелец видел бы
    /// одну команду, а исполнилась бы другая.
    #[test]
    fn invisible_separators_are_refused_everywhere() {
        // Zl, Zp, C1, Cf — невидимы; обычный и неразрывный пробелы — нет.
        for ch in ['\u{2028}', '\u{2029}', '\u{0085}', '\u{200b}', '\u{202e}', '\u{00ad}', '\u{feff}', '\t'] {
            assert!(broker_invisible(ch), "{:04x} обязан считаться невидимым", ch as u32);
        }
        for ch in [' ', '\u{00a0}', 'a', 'я', '—'] {
            assert!(!broker_invisible(ch), "{:04x} — видимый знак", ch as u32);
        }

        let ask = |cmd: &str, arg: &str, why: &str| {
            BrokerAsk::parse(&serde_json::json!({
                "v": BROKER_V, "token": "deadbeefdeadbeef", "op": "exec",
                "cmd": cmd, "args": [arg], "why": why,
            }).to_string())
        };
        // Чистая просьба проходит — граница не ломает работу.
        assert!(ask(r"C:\Windows\System32\netsh.exe", "advfirewall", "правило для телефона").is_ok());

        let why_bad = ask(r"C:\Windows\System32\netsh.exe", "advfirewall", "правило\u{2028}delete all")
            .expect_err("U+2028 в «зачем» обязан быть отказом");
        assert!(why_bad.contains("зачем"), "{why_bad}");

        let arg_bad = ask(r"C:\Windows\System32\netsh.exe", "advfirewall\u{2028}delete", "правило")
            .expect_err("U+2028 в аргументе обязан быть отказом");
        assert!(arg_bad.contains("невидимые знаки"), "{arg_bad}");

        let cmd_bad = ask("C:\\Windows\\System32\\net\u{200b}sh.exe", "a", "правило")
            .expect_err("ноль-ширины в имени программы обязан быть отказом");
        assert!(cmd_bad.contains("невидимые знаки"), "{cmd_bad}");

        // Показ подделать нельзя даже тем, что граница пропустить не должна.
        let shown = broker_shown_command("C:\\a.exe", &["раз\u{2028}два".into(), "три\u{202e}".into()]);
        assert!(!shown.contains('\u{2028}'), "{shown}");
