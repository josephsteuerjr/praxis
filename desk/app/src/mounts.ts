  ...
  const drawList = () => {
    ...
    for (const row of rows) {
      ...
      // шум. О самой слепоте сказано один раз, в блоке просьб.
        ...
        line.append(el("p", "field-hint", "В снимке кода агента этой папки ещё нет — появится после сохранения и перезапуска."));
      }
      listBox.append(line);
    }
  };
  ...
  box.append(
    ...
    el(
      ...
      "Смонтированная папка открывается агенту целиком: он видит её и внутри своего дома, как mnt/<имя>. " +
        "Годится ли путь, решает код агента — системную папку, корень диска и папку самой Hélène он не откроет и скажет почему. " +
