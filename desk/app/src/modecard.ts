...
export function modeCard(
  ...
  if (!live || !live.name || !fences.length) {
    const why = failure
      ...
      : !live || !live.name
        ...
        : "Канал ответил, но списка режимов не прислал — выбирать не из чего. Похоже, программа агента старее окна.";
  ...
  const syncNow = () => {
    ...
    if (legacyPipe) {
      ...
      now.textContent = `Сейчас: ограда не названа — программа агента отвечает старой картиной, где режимом считалась служба. Служба Windows: ${svc}.`;
      return;
    }
    ...
    if (legacyPipe) {
      planBox.append(
        el(
          "p",
          "receipt err",
          "Эта программа агента ещё считает службу режимом — старая картина, в которой ограда и служба были одним списком. " +
  ...
  if (svcText) svcBox.append(el("p", "choice-text", svcText));
    ...
    svcBox.append(
      el(
        "p",
        "receipt err",
        "Про службу программа агента ничего не рассказал — похоже, он старее окна. Ставить и снимать её кнопками " +
  ...
  for (const t of option?.toggles || []) {
    ...
    if (!hand) {
      row.append(
        el("p", "receipt err", `${t.title || t.key}: эту тумблер окно писать не умеет — программа агента новее окна.`),
    ...
    if (s0) {
      ...
      s0.note.textContent = !session0
        ...
        : installed === false
          ...
          : "Включено: брокер будет выполнять команды агента правами системы, а программа агента поднимется под службой, без рабочего стола.";
    }
