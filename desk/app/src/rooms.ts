...
export async function renameRoom(room: Room, title: string): Promise<void> {
  ...
  try {
    ...
    if (!unsupported(e)) throw e;
    S.roomsUnsupported = true;
    throw new Error("Этот харнесс ещё не умеет переименовывать чаты: нужна версия с контрактом комнат (0.3.3).");
  }
}
...
export async function deleteRoom(room: Room): Promise<void> {
  ...
  try {
    ...
    if (!unsupported(e)) throw e;
    S.roomsUnsupported = true;
    throw new Error("Этот харнесс ещё не умеет убирать чаты: нужна версия с контрактом комнат (0.3.3).");
  }
}
