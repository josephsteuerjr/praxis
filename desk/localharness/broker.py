...
#: Брокер есть только на Windows: просьбы о правах исполняет служба
#: (`svc`), а порт на macOS — основа без службы и брокера. Стенды подменяют
#: флаг, чтобы разбирать тул на любой машине.
HAS_BROKER = os.name == "nt"
...
def install(agent_mod, tree: Path, cfg: dict | None = None) -> None:
    ...
    """
    if not HAS_BROKER:
        # Брокера в порте нет по замыслу (основа без службы и брокера): просьбы
        # о правах исполняла служба Windows, здесь их подписывать некому. Тул
        # не выдаём: обещать модели тул, который откажет всегда, хуже, чем не
        # иметь его. Секции в снимке нет — только строка в журнал.
