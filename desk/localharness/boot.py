...
PORT_DEFAULTS: dict[str, str] = {
    ...
    # ⚠ 1.0.1: её сон. `sleep.run_scheduled` в ядре заводят часы mtproto_runner, которого
    # харнесс не запускает, — до 1.0.1 ночного цикла (консолидация, формирование, ревизия
    # характера, карта памяти, мётла inbox) в издании не было вовсе. Теперь его зовёт
    # `runner._sleep_due` между ходами по её `sleep.due`; `off` — вернуть прежнее.
    "PRAXIS_SLEEP_CYCLE": "on",
}
