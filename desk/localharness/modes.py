...
#:
#: ⚠ FileVault отсюда убран осознанно (план §6, 19.09): «диск заперт до первого
#: входа» — свойство ЧУЖОЙ настройки, которую владелец, может, и не включал, а
#: на экране оговорка стоит у каждого. Это место документов
#: (`ПЕРВЫЙ-ЗАПУСК.md`, `HELENE-MAP.md`), и там оно осталось.
...
def resolve(cfg: dict, *, installed: bool | None = None) -> dict:
    ...
    if service_here and not toggles_here and stored_session0:
        # ⛔ «На этой системе нет» — запрещённая на экране форма: владельцу важно
        # не чего у него нет, а что делает ключ, который лежит в его файле.
        notes.append("ключ service.session0 здесь ничего не меняет: служба идёт "
                     "от твоего имени, права администратора спрашивает система "
                     "паролем. Ключ можно стереть из helene.json")
