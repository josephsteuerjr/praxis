...
SERVICE_TEXT = ("Ставится один раз, под администратором. Программа агента поднимается "
                "сам, как только ты вошёл в систему, — окно для этого держать "
                "не нужно; из системы вышел — программа агента закрылся вместе с "
...
SESSION0_TEXT = ("Программа агента поднимается под самой службой: права системы, "
...
COMPUTER_TEXT = ("Рука `computer`: окна, экран, клавиатура и мышь, файлы и "
                 ...
                 "(helene-body.exe), которое программа агента поднимает рядом с собой в "
...
def resolve(cfg: dict, *, installed: bool | None = None) -> dict:
    ...
    if where == "mode":
        notes.append(
            ...
            "программы агента (local|remote): в таком файле окно не поднимает программа агента, "
