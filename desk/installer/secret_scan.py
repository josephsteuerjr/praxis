...
# ⚠ Вывод — в UTF-8, и это не косметика. В голой консоли владельца (cp1251) прибор падал
# `UnicodeEncodeError` на знаке ⚠ — то есть ИМЕННО НА НАХОДКЕ, в единственном случае,
# ради которого он написан. Код возврата оставался честным, а строка «СЕКРЕТЫ В ЗЕРКАЛЕ —
# публиковать НЕЛЬЗЯ» до экрана не доезжала: человек видел traceback и читал его как
# «прибор сломался». При чистом дереве ⚠ в выводе нет, и прибор в той же консоли доходил
# до конца — то есть «нашёл секрет» и «упал с ошибкой» выглядели одинаково.
for _stream in (sys.stdout, sys.stderr):
    try:
        _stream.reconfigure(encoding="utf-8", errors="replace")
    except (AttributeError, ValueError):  # перенаправлен в файл/трубу — там уже байты
        pass
...
def main() -> None:
    ...
    # ⚠ ПРИБОР ОТКАЗЫВАЕТСЯ ПРИ НУЛЕ, А НЕ ПЕЧАТАЕТ ЗЕЛЁНОЕ. Поймано приёмкой 11.09
    # живым прогоном: подай не тот формат входа — и сверка честно скажет «снято: 0», а
    # следом «ноль попаданий: ни одно живое значение в зеркало не уехало» и выйдет нулём.
    # То же самое при «снято: 3, из них проверяется: 0». Оба раза вердикт «чисто» ничем
    # не обеспечен: не нашли не потому, что нет, а потому, что не искали.
    # Смотреть глазами на «снято: N» — не защита: она была предписана памяткой и этого
    # случая не ловила, потому что первое число ненулевое. Проверяет ВТОРОЕ.
    if not checked:
        raise SystemExit(
            f"НЕЧЕГО ИСКАТЬ: из {len(wanted)} снятых значений к проверке годится ноль — "
            f"это отказ прибора, а не чистое зеркало. Проверь формат входа "
            f"({SECRETS}): ждём либо json «источник → текст», либо построчный список "
            f"значений (`installer/secret-strings.txt`)."
        )
