...
fn plan_usable(plan: &Plan) -> Result<(), String> {
    if plan.mode != "local" {
        return Err(format!(
            "helene.json просит mode=\"{}\" — код агента живёт не здесь, служба не нужна",
            plan.mode
        ));
    }
    ...
    if let Some(runner) = &plan.runner {
        if !runner.exists() {
            return Err(format!("нет движка: {} (ключ \"runner\")", runner.display()));
        }
    }
    Ok(())
}
    ...
    while !stop.load(Ordering::Relaxed) {
            ...
            for id in &ids {
                ...
                if port_noted.is_none_or(|t| now.duration_since(t) >= Duration::from_secs(60)) {
                    let whose = match harness_holder(any.port, &any.token) {
                        ...
                        Holder::Guarded => {
                            "код агента под ключом, которого у меня нет — детей не поднимаю"
                                .to_string()
                        }
                        Holder::Silent => "это не код агента — детей не поднимаю".to_string(),
                    };
            ...
            .filter(|k| k.role != Role::Trube && port_busy_ids.contains(&k.agent))
        {
            if let Some(child) = &mut kid.child {
                ...
                log.line(&format!("{said}: остановлен — порт {port} держит другой код агента"));
            }
        }
...
// Родные утилиты Windows (schtasks, icacls, netsh) отвечают не в UTF-8, а в
// кодовой странице консоли системы. Без перевода причина отказа приезжала в
// `service.log` сплошными «□», и владелец, для которого этот файл и написан,
// не мог прочитать ни слова. Теперь один код на все три программы.
...
fn supervise_session(plan: &Plan, stop: Arc<AtomicBool>, log: &mut Log, stopping: &mut dyn FnMut()) {
    log.line(&format!(
        "служба: код агента живёт в сессии владельца (задача {SESSION_TASK}) · дерево={} · порт={} · телефон={}",
        plan.tree.display(),
        plan.port,
        if plan.phone { "да" } else { "нет" }
    ));
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        if alive {
            ...
            if said != Said::Alive {
                log.line("код агента отвечает на порту — не вмешиваюсь");
                said = Said::Alive;
            }
            backoff = 0;
            next_try = now;
                ...
                Some(user) => {
                    if now >= next_try {
                        match schtasks(&session_task_run_args()) {
                            Ok((true, _)) => {
                                log.line(&format!("прошу планировщик поднять код агента в сессии {user}"));
                                said = Said::Trying;
                            }
    ...
    match mode.as_str() {
        ...
        "session-host" | "foreground" => {
            ...
            if session {
                ...
                drop_console();
                log.line("код агента поднят в сессии владельца (задача планировщика, без повышения)");
            }
            let stop = Arc::new(AtomicBool::new(false));
