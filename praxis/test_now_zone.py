...
#   провенанс evidence ........... 1128  заголовки + легенда + подписи 893 − экономия на
...
#
# ⚑ СДВИГ 09.08 РАЗЛОЖЕН ПОШТУЧНО, а не списан на «код поехал» — прежний абзац выше честно
# признаёт, что вклад причин тогда НЕ мерился, и повторять это второй раз незачем. Обе
# причины — правки контракта досье и почты, обе проверены подменой одного файла за раз:
#
#   +61  два слота отбора в `frame_layout._TIERS` переписаны («все и целиком, без отбора» →
#        «присутствующие по транспорту целиком; упомянутые — указателем»; «дельта с прошлого
#        пульса» → «при почтовом событии или обращении; иначе локатор»). Слот отбора живёт
#        ТОЛЬКО в строке провенанса, поэтому попадает и в дельту кадра, и в подписи.
#   −87  ярлык почтового тира стал локатором: «Почтовый ящик (свежий; действуй тулами
#        mail_read / mail_draft_reply / send_email, поллить не можешь)» → «Почтовый ящик —
#        ЛОКАТОР, не индекс». Ярлык стоит и под рычагом отката, поэтому в ДЕЛЬТЕ кадра
#        сокращается сам с собой и виден только в подписях.
#
