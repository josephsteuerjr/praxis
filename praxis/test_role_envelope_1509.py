...
class Rows(unittest.TestCase):
    ...
    def test_epoch_rows_make_the_same_decision(self):
        rows = group_context.epoch_rows(self.PEER, since_message_id=1, topic_id=None,
                                        whole_room=True)
        own = self._own(rows)
        self.assertEqual(own["role_line"], "мой ответ")
        self.assertIn("Praxis [id 777]", own["line"])
    ...
    def test_foreign_rows_are_identical_in_both_renders(self):
        for rows in (group_context.context_rows(self.PEER, topic_id=None, limit=50),
                     group_context.epoch_rows(self.PEER, since_message_id=1,
                                              topic_id=None, whole_room=True)):
            for row in rows:
