"""Накопитель уведомлений: что случилось в другой комнате, пока она занята (25.09, поток G).
...
def block_for_input(*, run_id: str, run_kind: str, chat_id, owner_context: bool,
    ...
    with _locked():
        ...
        _save(items)
    head = "Пока ты занята" + (f" ({busy_label})" if busy_label else "") + ":"
    if mid_turn:
        head = "Пока ты работала, пришло:"
