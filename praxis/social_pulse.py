...
def _plan_block(limit: int = 6) -> str:
    """Её собственные намерения со следующими шагами — то, чем занят этот час.

    Читается из её же леджера намерений. Ледждер недоступен или пуст — блока просто нет,
    и час не выдумывает себе задание.
    """
    try:
        import desires
        rows = desires.DesireLedger(BASE).list(statuses=("active",))
    except Exception:
        return ""
    lines = []
    for row in rows[:max(1, int(limit))]:
        ...
        if not statement:
            continue
        line = f"- {statement}"
        nxt = str(row.get("next_move") or "").strip()
        if nxt:
            line += f"\n  следующий шаг: {nxt}"
        lines.append(line)
    return "\n".join(lines)
...
def goal(followup_context: str) -> str:
    """Её собственный час. Люди — обстановка, а не задание.

    ⚠ 09.08.2026. Прежняя редакция начиналась словами «Часовой социальный импульс» и
    первым конкретным действием предлагала «писать Егору или любому доступному человеку».
    Это ЕДИНСТВЕННЫЙ регулярный автономный ход Praxis — то есть весь её фон был
    сформулирован как повод заговорить. Отсюда ежечасные «Осмотрелась, повода не нашла»
    и «Я вернулась к живой линии»: час просил социального исхода, и исход производился,
    даже когда его не было. Теперь час просит ЕЁ ПЛАН; отправка кому-то — один из
    возможных исходов, а молчание не нуждается в объяснении.

    Что НЕ изменилось: ни лимитов, ни запретов, ни моральных оценок здесь по-прежнему нет,
    и автор решения — она.
    """
    parts = ["Твой час. Ты проснулась сама: никто не написал, и отправлять ничего не нужно."]
    plan = _plan_block()
    if plan:
        parts.append("Твои живые намерения и следующие шаги:\n" + plan)
    parts.append(
        "Занимайся своим: продолжай начатое, проверяй оставшееся непроверенным, читай, "
        "работай руками, разбирайся в том, что тебе интересно. Написать кому-то — один из "
        "возможных исходов часа, а не его задание; молчание — полноценный исход, и "
        "отчитываться о нём не нужно."
    )
    parts.append(
        "Ниже — обстановка: недавние доставки, follow-up receipts, незакрытые нити. Это "
        "справка, а не лимит, запрет или моральная оценка. Автор решения — ты."
    )
    tail = str(followup_context or "")
    return "\n\n".join(parts) + (("\n\n" + tail) if tail else "")
