...
enum: меняется ТОЛЬКО пояснительный текст. Значит поведение рук не трогается вовсе, а
отпечаток набора (`tool_offerings.fingerprint`) меняется один раз, на включении.
...
EN: dict[str, dict] = {
    ...
    "reply": {
        "d": ("Answer the interlocutor of this conversation. Your reply reaches a person "
              ...
              "nowhere and it closes the turn. So: you called this hand — the message is "
    ...
    "say": {
        "d": ("Look at your own reply before it goes out. Returns your own text and one "
              "fact: which hands were used in this turn. It sends nothing and demands "
    ...
    "stay_silent": {
        "d": ("Deliberately stay silent, with a short reason recorded for yourself. In a "
              ...
              "background window there is nothing to hold: there you send by hand. If you "
    ...
    "search_private_messages": {
        "d": ("Explicitly search text across your own Telegram DMs. Nothing from DMs enters "
              "ordinary context by itself: this hand runs only when you actually need the "
    ...
    "freeze_chat": {
        "d": ("Freeze (on=true) or unfreeze (on=false) a chat — a frozen one does not reach "
              ...
              "the owner and to Praxis herself; provenance is written as owner or praxis."),
    ...
    "inbox_read": {
        "d": ("Read a text Telegram file inside workspace/inbox with line numbers. A "
              "read-only hand for files that may have been sent earlier or not to you "
              "personally."),
    ...
    "reconcile_run": {
        "d": ("Your hand on a run stuck in in_doubt. With no arguments: show all in_doubt "
    ...
    "consolidate_context": {
        "d": ("Fold the older part of the available history into the journal without losing "
              ...
              "source; in live Telegram the role history can only be a snapshot, and then "
    ...
    "git": {
        "d": ("Your git with both hands. repo=self is the tree you live in; repo=public is "
