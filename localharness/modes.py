...
TEXTS = {
    ...
    "interactive": ("Агент работает с твоими правами: файлы и процессы — те же, "
                    "что доступны тебе самому, не больше. Если учётка "
                    "ограничена, агент ограничен так же. Права администратора "
                    "он просит отдельно, окном Windows."),
}
...
SERVICE_TEXT = ("Ставится один раз, под администратором. Харнесс поднимается "
                "сам, как только ты вошёл в систему, — окно для этого держать "
                "не нужно; из системы вышел — харнесс закрылся вместе с "
...
SESSION0_TEXT = ("Харнесс поднимается под самой службой: права системы, "
...
def resolve(cfg: dict, *, installed: bool | None = None) -> dict:
    ...
    if where == "mode":
        notes.append(
            ...
            "харнесса (local|remote): в таком файле окно не поднимает харнесс, "
