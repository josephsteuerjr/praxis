    ...
    try:
        ...
        if not k.AssignProcessToJobObject(job, handle):
            log.warning("тело: %s не приписан к заданию (код %d) — после аварии "
                        "раннера может остаться сиротой", proc.args[0],
                        ctypes.get_last_error())
    except Exception:
...
class Body:
    ...
    def stop(self) -> None:
        ...
        STATE.update({"connected": None, "bridge_pid": 0, "body_pid": 0,
                      "reason": "остановлено вместе с раннером"})
        try:
            self._write_state()
        except Exception:
            pass
...
def windows_truth() -> str:
    ...
    return (f"окна: тело поднято харнессом снаружи ограды (мост 127.0.0.1:{STATE.get('port')}, "
