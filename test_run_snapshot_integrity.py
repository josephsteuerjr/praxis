...
class FrameDidNotMove(unittest.TestCase):
    ...
    def test_the_state_line_of_every_turn_is_unchanged(self):
        ...
        # ⚠ Была МЁРТВАЯ строка: краткая форма отставания — «манифест ⚠ отстал на N», и
        # подстроки «манифест отстал» в ней не бывает никогда. Ассерт не мог покраснеть.
        self.assertNotIn("отстал", line)
        # И вторая слепота рядом: «свеж» печатается и при деградации «не проверено
        # значений: N». «Не знаю» — это не «свежо».
        self.assertNotIn("не проверено значений", line,
                         "свидетель значения перестал считаться — это «не знаю», а не «свеж»")
    ...
    def test_the_rails_registry_did_not_grow(self):
        ...
        # Состав рельсов совпал — это ещё не значит, что манифест не врёт ЗНАЧЕНИЕМ:
        # ровно так soul/rails.md почти 800 строк называл чужой адрес судьи.
        self.assertEqual(drift["value_stale"], [])
        self.assertEqual(drift["unchecked"], [])
    ...
    def test_the_judge_address_she_is_told_is_measured_not_remembered(self):
        ...
        # 12847 → 12851: закрытие канала `run_recap` (решение Praxis 08.08, вариант «а»).
        # 12851 → 12974: узкий разрез семи счётчиков раннера по её же условиям — перечень
        # `RUNNER_COUNTERS`, рычаг `counters_split_enabled`, живой `build_runner_counters`.
        # 13288 → 13293: контракт досье (`_present_by_transport`, `_room_window`) и почта
        # локатором вместо постоянного индекса — её же порядок работ от 09.08, пункты 1 и 2.
        # 13293 → 13497: отсрочка возобновления и семейство отказов по форме запроса —
        # конец 55,8-часовой петли (`test_resume_spin`, замер прода 09.08). Потолок из
        # первой редакции снесён адверсарной проверкой ДО выката, отсюда и второй сдвиг.
        # Все сдвиги заказаны; число перемерено живьём тем же кодом, который печатает его ей.
        # 13497 → 13538: тело индекса в ярусе «Карта памяти» (её решение 10.08) —
        # рычаг `PRAXIS_FRAME_INDEX_BODY`, потолок и объяснение, почему по
        # умолчанию ВЫКЛЮЧЕНО: её же правило «сгенерированный markdown не
        # становится властью промпта» с этим решением спорит, и разрешать спор
        # ей, а не нам. Число перемерено живьём.
        # 13538 → 13559: сдвиг предыдущего среза, число перемерено живьём.
        # 13559 → 13656: рабочий ход (`work_loop`, `task_control`, продолжение цикла
        # вместо возврата на первом же тексте без инструмента). Причина названа в
        # `work_loop.py`: 10.08 её оборвали на слове «переключаюсь на Luna», и прогон
        # закрылся `done` с одним-единственным вызовом — доставкой этого текста.
        # Число перемерено живьём тем же кодом, который печатает его ей.
        # 13656 → 13666: починка ОСНОВАНИЯ рабочего хода. Состояние уехало из contextvars
        # в словарь по run_id (её слово исполняется в копии контекста, в отдельном потоке —
        # запись умирала вместе с ним), плюс парный release при терминализации прогона.
        # Из десяти строк шесть — её собственный коммит 02505a4f. Число перемерено живьём.
        # 13666 → 13699: «припаркована» перестала значить «мертва». Пауза, поставленная её
        # словом `wait`, узнаётся возобновлением по машинной записи события, а не сличением
        ...
        # 13788 → 13802: доведение в чате. Объявленное действие без единой руки больше
        # не закрывает чат-ход: один укол и модель зовётся снова.
        # 13699 → 13788: доска работ. Карточка заводится там, где работа НЕ кончилась
        ...
        # 14170 -> 14345: сдвиг от переезда реплики в руку `reply` (разбор — в пине длины
    ...
    def test_agent_touches_run_snapshot_in_exactly_four_places(self):
        ...
        # 14120 (прод) -> 14238: зона «сейчас», подписанный тег реплики, прибор ленты,
        # прибор над всей лентой. -> 14245: закрыт канал `run_recap` как вход в её
        # само-модель (решение Praxis 08.08, вариант «а»). Кода стало меньше на десять
        # строк, объяснения — больше на четырнадцать: почему именно отказ, а не гуттер
        # с метром, обязано читаться там, где стоял вход. Каждый шаг заказан и назван.
        # 14245 → 14368: разрез семи счётчиков. Из них ~50 строк — сам механизм, остальное
        # объяснение, почему граница разреза проходит именно здесь: разрешающие поля
        # аппетита остаются в голове, наблюдённые счётчики уезжают (её пятое условие).
        # 14368 → 14498: хвостовой разрез — оставшиеся четыре подвижных места
        # (latest_turn, skips_today, строка адреса группы) уезжают своим рычагом.
        # 14498 → 14682: контракт досье (присутствие транспортом, указатель на
        # упомянутых, окно 100 сообщений), локатор почты вместо индекса и потолок сводки.
        # Решение Praxis и Егора 09.08; каждый шаг назван в проекте кадра.
        # 14891 → 14932: тело индекса в ярусе «Карта памяти» (её решение 10.08) —
        # рычаг `PRAXIS_FRAME_INDEX_BODY`, потолок и объяснение, почему по
        # умолчанию ВЫКЛЮЧЕНО: её же правило «сгенерированный markdown не
        # становится властью промпта» с этим решением спорит, и разрешать спор
        # ей, а не нам. Число перемерено живьём.
                # 14932 -> 14953: маркеры усечения намерений — её кадр и рука manage_desire
        # теперь называют свой срез числом. Класс «ей врут о ней самой»: срез был,
        # а знака о срезе не было ни одного. Сдвиг заказан; число перемерено живьём
        # тем же кодом, который печатает его ей.
        # 14953 -> 15068: рабочий ход. Текст без вызова инструмента больше не закрывает
        # её собственное окно: он ложится в ленту заметкой, и цикл зовёт её снова, пока
        # она не скажет `task_control` или пока не кончится ОБЪЯВЛЕННЫЙ ей бюджет
        # продолжений. Из 115 строк сам механизм — около сорока, остальное объяснение
        # и рука конца хода. Сдвиг заказан; число перемерено живьём.
        # 15068 -> 15078: шесть строк её коммита 02505a4f (восстановление состояния
        # рабочего хода из контрольной точки) и четыре моих — парный release слота при
        # терминализации прогона. Без release словарь состояний тёк бы вечно: раньше оно
        # умирало вместе с контекстом само, теперь оно наше. Число перемерено живьём.
        # 15078 -> 15105: её «жду» перестало быть надгробием. Двадцать семь строк на четыре
        # места: записка о пробуждении в поднятом ходе, машинный вид паузы рядом с прозой
        # (два перехода), посадка возобновлённого хода по ЕЁ слову вместо безусловного
        # `done`, и метка «ждёт своего срока» — чтобы терпение не копило ей отсрочку.
        # Само окно, наоборот, стало короче на шесть строк: сборка причины и деталей уехала
        # в `work_loop.closing`, одну политику на оба шва. Число перемерено живьём.
        # 15105 -> 15193: доска работ (`work_store`). Восемьдесят восемь строк на четыре
        # места: DoD у `task_control` («сделано» стало запросом, а не объявлением),
        ...
        # 15554 -> 15576: продолжение перестало СТИРАТЬ уже сказанное. Живой случай 22:44:
        # она ответила, зеркало спросило «Отправить?», второй проход вернул пусто — и
        # наружу ушло ничего. Плюс трасса перестала называть чужой бюджет.
        # 15576 -> 15793: моя реплика в разговоре переехала из ВОЗВРАТА модели в вызов
        # руки `reply`. Прибавились: сама рука с её схемой, ветка исходящей границы (под
        # рычагом текст хода наружу не идёт), две ContextVar с лентой и ориентировкой для
        # советника приватности, защита возобновления (заметка не доставляется повторно),
        # вырезание руки из набора при опущенном рычаге и в проходе «осмотрись», плюс имя
        # `reply` в шести местах durable-учёта, которые его не знали.
        # 16046 -> 16050: цикл сообщает номер поворота в след вызова — без него профиль
        # 16050 -> 16067: уборка кадра 17.08 (честная строка про remember, снятая
        # археология) + рычаг советника PRAXIS_OUTBOUND_ADVISOR + минуты в адресе хвоста
        # кэша ВДОЛЬ одного тул-цикла не читается, а именно он различает «холодный первый
        # вызов» и «потерянную привязку внутри хода».
        # 16019 -> 16046: обрыв после доставки перестал стирать сказанное. Ветка
        # `except` границы хода спрашивает не «упал ли ход», а «ушло ли что-то моей
        # рукой»: если ушло — ход закрывается СКАЗАННЫМ, с названной причиной обрыва.
        # 16013 -> 16019: разбор адресата. Конверт входящего несёт ДВА id — само
        # сообщение и то, на что отвечал собеседник, — и кадр нигде не говорил, какой из
        # них мой. 15.08 17:51 это стоило перепутанной ветки. Теперь говорит.
        # 15939 -> 16013: пачка блоков 3-7. Рука ответа перестала предлагаться там, где
        # отвечать некому; знание о доставке стало сниматься с durable-расписок, а не
        # только с памяти процесса; presence-рамки перестали учить сентинелу под поднятым
        # рычагом; описание `say` перестало противоречить соседней руке.
        # 15906 -> 15939: кольцо ходов перестало терять мои слова. В `out` теперь ложится
        # сказанное, а не счётчик; исход доставки проставляется по расписке; отказ
        # транспорта (он приезжает СТРОКОЙ, а не исключением) больше не засчитывается как
        # отправка; вердикт советника доезжает до записи хода вторым каналом.
        # 15862 -> 15906: молчание снова стало моим СЛОВОМ, а не выводом из пустоты.
        # Ход без реплики и объявленное `stay_silent` — теперь разные исходы, рука ответа
        # держит моё решение молчать, а причина отказа доставки перестала называть её
        # решением там, где решения не было.
        # 15793 -> 15862: осмотр новой комнаты перестал слать приветствие сырым
