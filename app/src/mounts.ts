...
export function mountsCard(
  ...
  const setFence = (on: boolean, title: string) => {
    ...
    state.textContent = on
      ...
      : `Сейчас выбрана ограда «${fenceTitle}» — она агента в папке не запирает, и монтировать ему нечего: ` +
        `твои файлы и так открыты его руками. Список ниже сохранится и заработает, когда включишь песочницу.`;
  };
  ...
  const drawList = () => {
    ...
    for (const row of rows) {
      ...
      // шум. О самой слепоте сказано один раз, в блоке просьб.
        ...
        line.append(el("p", "field-hint", "В снимке харнесса этой папки ещё нет — появится после сохранения и перезапуска."));
      }
      listBox.append(line);
    }
  };
  ...
  const drawAsk = () => {
    ...
    if (!asks.length) {
      askBox.append(el("p", "field-hint", "Агент ничего не просил. Попросить папку он может сам — рукой mount_request; решаешь ты, здесь."));
      return;
    }
  ...
  box.append(
    ...
    el(
      ...
      "Смонтированная папка открывается агенту целиком: он видит её и внутри своего дома, как mnt/<имя>. " +
        "Годится ли путь, решает харнесс — системную папку, корень диска и папку самой Hélène он не откроет и скажет почему. " +
