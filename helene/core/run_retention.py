...
#: Windows-тег «суррогат имени» (IsReparseTagNameSurrogate): симлинк, junction, точка
#: монтирования. У облачных заглушек (OneDrive Files On-Demand) этого бита нет.
_NAME_SURROGATE = 0x20000000


@contextlib.contextmanager
def _windows_evidence_stream(path: Path):
    """Чтение улик в издании на Windows — ⚠ отличие издания (ревью W2 S1, 26.09).

    Здесь нет `openat`/`O_NOFOLLOW`/`dir_fd`, и ядерные ворота выше отвечали отказом на
    КАЖДОЕ чтение: ни один ход не закрывал прогон (`read_result` → расписка доставки),
    рука `read_run_result` и возобновление не читали результатов. Издание живёт на личном
    компьютере — одна машина, один пользователь, дерево в его профиле (тот же довод, что у
    `runs_prune._open_work_runs_here`). Проверяется то, что проверяемо без openat:
    в пути под корнем прогонов (`…/runs/…`) нет ссылок и junction, файл — обычный и с одной
    жёсткой ссылкой; отсутствие только последнего звена — `EvidenceNotFound`, как у ядра.
    Выше `runs` ссылки не проверяются: перенесённый профиль или папка данных на junction —
    законная раскладка личной машины, а не подмена улик.
    """
    descriptor = None
    try:
        ...
        low = [part.casefold() for part in parts]
        start = (len(low) - 1 - low[::-1].index("runs")) if "runs" in low else len(parts) - 1
        for depth in range(max(1, start), len(parts)):
            probe = Path(*parts[:depth + 1])
            try:
                info = os.lstat(probe)
            ...
            if stat.S_ISLNK(info.st_mode) or (
                ...
                raise ContractError(f"evidence path is redirected at {probe.name!r}")
...
def safe_evidence_stream(path: Path):
    ...
    """
    if not _SAFE_OPENAT_SUPPORTED and os.name == "nt":
        with _windows_evidence_stream(path) as stream:
            yield stream
        return
