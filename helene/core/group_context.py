...
def _format_message(row: dict, *, max_text: int = 1200,
    ...
    `as_self` — ЕЁ собственная реплика так, как она едет в модель ролью `assistant`:
    ТЕЛО без машинного конверта. Конверт в этой позиции был не её координатами, а нашей
    разметкой архива, и он отвечал на вопросы, на которые роль уже ответила: кто говорит —
    сказано ролью, когда — порядком, кому — строкой выше. В АРХИВНОМ виде (`line`, рука
    `group_context`) конверт остаётся байт-в-байт: там роли нет, и там он вправду адрес.

    ⚠ 15.09.2026, ПОЧЕМУ ЭТО ИЗМЕНИЛОСЬ — ИЗМЕРЕНО, А НЕ РЕШЕНО ПО ВКУСУ. Раньше здесь
    печаталась та же шапка без имени: `[root; message #8; 2026-…Z; reply_to=#7] текст`.
    В кадре она стояла ПЯТЬЮ ходами подряд и читалась как образец ответа. glm-5.3 образец
    воспроизводил: выдавал обычный текст с ВЫДУМАННОЙ шапкой и выдуманным номером
    (`[root; message #105408; …] Пробую`), не звал `reply` вовсе, и наружу не уходило
    ничего. За сутки 15.09 таких ходов четырнадцать, все на glm, ни одного на
    sol/astra/terra. Проба на том же кадре, ступень low: с шапкой рука звалась не в
    каждом заходе, без шапки — в каждом. Форма кадра учила модель тому, чего контракт
    речи запрещал словами; слова проигрывают образцу.

    ⚠ ИЗДАНИЕ: путь недостижим на шипаемых поверхностях. Транспорт подменяет руку
    (`localharness/transport.py::_honest_group_context`), архив пишется одним файлом, а
    лента берётся из `memory_life.hot_records` — машинного конверта в ленте издания нет
    ни на одной поверхности. Правка перенесена ПАРИТЕТОМ, чтобы файл не разъехался с
    ядром молча; замер после порта её вклада не покажет, и это ожидаемо.
    """
    topic = row.get("topic_id")
    ...
    if as_self:
        # Её слова — и только они. Ни один символ конверта не уезжает в роль
        # `assistant`: там он был образцом для подделки, а не координатой.
        return body
    head = (f"[{topic_mark}; message #{row.get('message_id')}; "
...
def context_rows(peer_id: str | int, *, topic_id: int | None, limit: int = 80,
    ...
    if marks:
        # ⚠⚠ ЧИСЛА СТОЯТ В ХВОСТЕ, А НЕ В НУЛЕВОЙ СТРОКЕ, И ЭТО ПРО ДЕНЬГИ.
        #
        # Пометка обреза вставлялась ПЕРВОЙ строкой ленты вместе со счётчиками, а
        # счётчики живые: за три подряд идущих хода в Грибнице они шли 1736 → 1740 →
        # 1750, показано 200 → 193 → 188. Первая строка — начало префикса, который
        # провайдер кэширует; сдвинулась она — и весь кадр за ней оплачивается заново.
        # Замер 27.08 по 143 парам соседних ходов: общий префикс ленты в комнатах был
        # РОВНО НОЛЬ сообщений, а без этой одной строки становится 6–10 (медиана), то
        # есть 20–32 тысячи знаков на ход. В личке пометки нет — там префикс и так цел
        # (48 сообщений), поэтому болезнь была видна только в комнатах.
        #
        # ⚑ САМ ФАКТ ОБРЕЗА ОСТАЁТСЯ НАВЕРХУ — ради него пометка и заведена: срез без
        # неё читается как «вот вся ветка». Наверх едет постоянный текст, вниз —
        # точные числа и совет, что с ними делать. Ничего не выкинуто и не округлено:
        # округлённый счётчик врал бы тихо, а это хуже, чем ждать числа строкой ниже.
        #
        # ⚑ НАЗВАННАЯ ЦЕНА: точные остатки теперь приезжают ПОСЛЕ ленты, а не на шве.
        # Строка обстановки, а не реплика (`self=False`), и адрес хода она не трогает:
        # кому отвечать, говорит отдельная строка кадра, а не «последнее сообщение».
        seam_line = ("…[ЛЕНТА ОБРЕЗАНА: выше есть ещё сообщения. Сколько именно и что "
                     "с этим делать — строкой в конце ленты.]")
        rest_line = (f"…[ОБРЕЗ ЛЕНТЫ, точные числа: показано {shown} сообщений, "
        ...
        lines.insert(0, {"self": False, "line": seam_line, "role_line": seam_line,
                         "service": True})
        lines.append({"self": False, "line": rest_line, "role_line": rest_line,
                      "service": True})
    if root_line:
