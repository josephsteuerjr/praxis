...
Local-filesystem checkpoint store; no live imports or provider execution.

Издание (26.09): тот же контракт на Windows. Замок — `exclusive()`: fcntl.flock там, где
он есть, msvcrt.locking на Windows (тоже исключительный межпроцессный замок, не
ослабление); fsync каталога на Windows не делается — открыть каталог дескриптором там
нельзя, а журналирование переименования — забота NTFS; fsync файла идёт через дескриптор на
запись — FlushFileBuffers требует права записи, и дескриптор на чтение давал EBADF.
"""
...
def _sync_directory(path):
    if not hasattr(os, 'O_DIRECTORY'):
        # Windows: каталог дескриптором не открывается; запись каталога после
        # os.replace журналирует NTFS, отдельного fsync у неё нет.
        return
...
def _sync_file(path):
    # Windows: os.fsync = FlushFileBuffers, которому нужен дескриптор с правом записи.
    with path.open('rb' if os.name != 'nt' else 'r+b') as stream:
        os.fsync(stream.fileno())


@contextmanager
def exclusive(path, *, timeout=120.0):
    """Исключительный межпроцессный замок файла `path` (создаётся при нужде).

    POSIX — fcntl.flock, как и было. Windows — msvcrt.locking на первый байт того же
    файла: тоже исключительный и тоже снимается ядром, если процесс умер. Ждём
    короткими шагами, а не по секунде (LK_LOCK), и не вечно: не взятый за `timeout`
    замок — отказ контракта, вызывающий уходит на прежний путь, а не висит.
    """
    import time
        ...
        while True:
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise ContractError('lock timeout')
                time.sleep(0.02)
        try:
            yield
        finally:
...
def validate(*, source_bytes: bytes, authority_bytes: bytes, pins: EvidencePins,
    ...
    for index, entry in enumerate(projection):
        ...
        _text(entry['issuer'])
        if entry['transform'] == RECEIPT_TRANSFORM:
            # Издание: расписка сборщика — без происхождения, но только точная и только
            # сразу за своим вызовом (keat_candidate.render_receipt).
            _require(entry['origins'] == [] and is_render_receipt(messages, index),
...
class EpochStore:
    """Immutable objects + atomic HEAD under a cooperative exclusive file lock.
    ...
    @contextmanager
    def _locked(self):
        # Explicit exclusive backend on every platform; never silently weaken locking.
