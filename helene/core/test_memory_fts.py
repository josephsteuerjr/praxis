...
class ExplicitSearchHotPathTests(unittest.TestCase):
    ...
    def test_poisoned_cached_locator_cannot_promote_excluded_markdown(self) -> None:
        ...
        for relative in ("_internal.md", "people/_internal.md",
            ...
            with self.subTest(relative=relative):
                ...
                # Издание: `with sqlite3.connect(...)` только коммитит и НЕ закрывает —
                # на Windows открытая база не даёт стенду убрать временную папку.
                with contextlib.closing(sqlite3.connect(memory_fts._db_path(self.memory))) as db:
    ...
    def test_refresh_request_has_one_cross_process_owner(self) -> None:
        ...
        # Издание: работник — функция модуля, а не замыкание: под spawn (Windows) замыкание
        # не пересылается в дочерний процесс, и гонка за владение не проверялась вовсе.
        children = [ctx.Process(target=_request_refresh_race,
                                args=(str(self.memory), gate, output)) for _ in range(2)]
...
def _request_refresh_race(memory, gate, output):
    gate.wait(3)
    output.put(memory_fts.request_refresh(memory_dir=Path(memory), reason="race"))
...
class RefreshClaimRecoveryTests(unittest.TestCase):
    ...
    def test_restore_link_failure_releases_ownership_and_recovers(self) -> None:
        ...
        claim = memory_fts.claim_refresh_request(memory_dir=self.memory)
        self.assertIsNotNone(claim)
        # Издание: на Windows заявка возвращается через rename (жёстких ссылок может не
        # быть), на POSIX — через link. Сбой подменяется в той функции, которую зовёт
        # платформа; иначе на Windows проверялся бы удачный возврат, а не отказ.
        failing = "rename" if memory_fts._WINDOWS else "link"
        with mock.patch.object(memory_fts.os, failing, side_effect=OSError("temporary I/O failure")):
