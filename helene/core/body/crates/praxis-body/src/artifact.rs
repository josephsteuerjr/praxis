...
impl ArtifactClient {
    pub fn new(config: &BodyConfig) -> Result<Self> {
        ...
        // A loopback bridge is a neighbour process, never a route: an `HTTP_PROXY` in the
        // environment would send artifact chunks out of the machine and back, or nowhere at
        // all. The websocket lane already dials raw TCP and never saw a proxy; this makes the
        // artifact lane agree with it. A bridge on a real host keeps the environment's proxy.
        if parsed_base.host().is_some_and(|host| match host {
