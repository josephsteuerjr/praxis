...
mod tests {
    ...
    /// Текст набирается по одной записи на отправку с паузой после отпускания:
    /// пачка VK_PACKET в одной отправке приезжает в Блокнот 11 как один и тот же
    /// последний знак (разбор — у TEXT_UNIT_PAUSE_MS). Пауза набора не входит в
    /// бюджет `max_total_delay_ms` — иначе абзац в 1600 знаков был бы отказом.
    #[cfg(windows)]
    #[test]
