...
mod platform {
    ...
    pub fn dispatch(capability: &str, args: Value, state_dir: &Path) -> Result<Value> {
        if capability != "desktop.status" && capability != "os.process.list"
            && !mac::gui_session()
        {
            bail!("the screen is currently in another user session; console=false; files and processes remain available")
        }
        match capability {
    ...
    fn desktop_status() -> Result<Value> {
        if !mac::gui_session() {
            return Ok(json!({
                "ok": true, "interactive": unsafe { libc::geteuid() } != 0, "console": false,
                "session_id": null, "platform": "macos",
                "foreground": null, "cursor": null, "virtual_screen": null,
                "scale": null, "tcc": null,
                "hints": ["the screen is currently in another user session; files and processes remain available"]
            }));
        }
        let tcc = mac::tcc();
        ...
        Ok(json!({
            "ok": true,
            "interactive": unsafe { libc::geteuid() } != 0,
            "console": true,
            "session_id": null,
...
mod platform {
    ...
    /// Пауза после КАЖДОГО знака текста (после отпускания), мс.
    ///
    /// ⚠ Почему текст не уходит одной пачкой. Знак Unicode едет как VK_PACKET, и
    /// получатель читает сам знак не в момент `SendInput`, а когда разбирает
    /// сообщение. Приложения с асинхронным вводом (Блокнот 11 / WinUI, TSF)
    /// разбирают очередь позже, чем она наполнилась, и читают ПОСЛЕДНИЙ знак:
    /// « — Мира была здесь» приезжало как пробел и семнадцать «ь» (06.09.2026,
    /// машина автора, Hélène). Проверено там же: пачка и пары down/up в одной
    /// отправке ломаются всегда; отдельные отправки с паузой 8–12 мс — иногда
    /// (пропущенный или сдвоенный знак); с 20 мс — стабильно. Классические
    /// edit-контролы терпят и пачку, но правда одна на всех: медленно и верно.
    /// Цена: ~50 знаков в секунду; в бюджет `max_total_delay_ms` пауза набора
    /// не входит и в ответе показана отдельно (`typing_pacing_ms`).
    const TEXT_UNIT_PAUSE_MS: u64 = 20;
    ...
    struct PreparedChunk {
        inputs: Vec<INPUT>,
        pause_ms: u64,
        /// Пауза набора текста (TEXT_UNIT_PAUSE_MS) — отдельно от `pause_ms`:
        /// её не просил вызывающий и в бюджет задержек она не входит.
        pacing_ms: u64,
    ...
    pub(super) struct PlanTotals {
        ...
        pub(super) records: usize,
        pub(super) pause_ms: u64,
        /// Сумма пауз набора текста — вне бюджета `max_total_delay_ms`.
        pub(super) pacing_ms: u64,
        pub(super) clamped_moves: usize,
    }
    ...
    fn input_perform(args: InputArgs) -> Result<Value> {
        ...
        let outcome = (|| -> Result<()> {
            for (index, chunk) in chunks.iter().enumerate() {
                ...
                let rest = chunk.pause_ms.saturating_add(chunk.pacing_ms);
                if rest > 0 && index + 1 < chunks.len() {
                    thread::sleep(Duration::from_millis(rest));
                    paused_ms = paused_ms.saturating_add(rest);
                }
            }
            Ok(())
        })();
        ...
        Ok(json!({
            ...
            "input_records": totals.records,
            "planned_pause_ms": totals.pause_ms,
            "typing_pacing_ms": totals.pacing_ms,
            "paused_ms": paused_ms,
    ...
    fn plan_totals(chunks: &[PreparedChunk]) -> Result<PlanTotals> {
        ...
        for chunk in chunks {
            ...
            pause_ms = pause_ms
                ...
                .context("input delay overflow")?;
            pacing_ms = pacing_ms
                .checked_add(chunk.pacing_ms)
                .context("typing pacing overflow")?;
            clamped_moves = clamped_moves
                .checked_add(chunk.clamped_moves)
                .context("clamped move count overflow")?;
        }
    ...
    /// Текст — по одной записи на отправку: нажатие, отпускание, пауза
    /// TEXT_UNIT_PAUSE_MS после отпускания. Разбор — у самой константы.
    fn text_chunks(text: &str) -> Result<Vec<PreparedChunk>> {
        let inputs = text_inputs(text)?;
        let mut chunks = Vec::with_capacity(inputs.len());
        for (index, input) in inputs.into_iter().enumerate() {
            let is_key_up = index % 2 == 1;
            chunks.push(PreparedChunk {
                inputs: vec![input],
                pause_ms: 0,
                pacing_ms: if is_key_up { TEXT_UNIT_PAUSE_MS } else { 0 },
                press: None,
                release: None,
                clamped_moves: 0,
            });
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn text_is_typed_one_record_per_batch_with_pacing_outside_the_delay_budget() {
        let plan = |value, delay| platform::plan_summary(&input_events(value), delay).unwrap();
        let typed = plan(serde_json::json!([{"type": "text", "text": "Мира"}]), 0);
        assert_eq!(typed.records, 8);
        assert_eq!(typed.batches, 8, "каждая запись — своя отправка");
        assert_eq!(typed.pause_ms, 0, "пауза набора — не задержка вызывающего");
        assert_eq!(typed.pacing_ms, 4 * 20);

        // Длинный текст не упирается в бюджет задержек: 1600 знаков × 20 мс > 30 с.
        let long = plan(serde_json::json!([{"type": "text", "text": "x".repeat(1_600)}]), 0);
        assert_eq!(long.pause_ms, 0);
        assert_eq!(long.pacing_ms, 1_600 * 20);
        // ...а задержка между событиями по-прежнему считается на стыке.
        let mixed = plan(
            serde_json::json!([{"type": "text", "text": "ab"}, {"type": "key", "key": "a"}]),
            100,
        );
        assert_eq!(mixed.pause_ms, 100);
        assert_eq!(mixed.pacing_ms, 40);
    }
    ...
    fn mac_live_status_answers_with_platform_and_tcc() {
        ...
        assert!(status["console"].is_boolean());
        if status["console"] == false {
            assert!(status["tcc"].is_null());
            assert!(status["virtual_screen"].is_null());
            assert!(status["hints"].as_array().is_some_and(|hints| !hints.is_empty()));
            return;
        }
    ...
    fn mac_live_window_list_and_process_list_answer_honestly() {
        ...
        assert_eq!(processes["ok"], true);
        if !crate::mac::gui_session() {
            let refused = dispatch("desktop.window.list", serde_json::json!({}), &state).unwrap_err();
            assert!(refused.to_string().contains("console=false"));
            return;
        }
        let all = dispatch(
            "desktop.window.list",
    ...
    fn mac_live_capture_gives_a_png_or_refuses_with_hints() {
        ...
        if !crate::mac::gui_session() {
            assert!(result.unwrap_err().to_string().contains("console=false"));
            return;
        }
        if tcc.screen_recording {
    ...
    fn mac_live_input_and_activation_refuse_without_accessibility() {
        ...
        if !crate::mac::gui_session() {
            assert!(input.unwrap_err().to_string().contains("console=false"));
            return;
        }
        if tcc.accessibility {
