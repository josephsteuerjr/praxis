...
/// macOS: a non-root body keeps its user execution identity when the console changes.
/// Console availability belongs to desktop.status and the desktop/AX operations,
/// not routing: macOS has no Windows session router to forward filesystem work to.
/// Privileges remain separate:
