...
# Windows: `os.replace` поверх файла, который в этот миг читает другой процесс (Пульт
# опрашивает manifest.json прогона каждые ~1.5 с), падает PermissionError (WinError 5/32,
# sharing violation) — и 08.09 это остановило живой ход Миры «durability failure during
# model intent persistence» посреди проверки. Читатель держит файл миллисекунды, поэтому
# короткие повторы закрывают гонку; после потолка ошибка честно уходит наверх.
_REPLACE_RETRIES = 12
_REPLACE_PAUSE_SEC = 0.04
...
def _replace_retrying(tmp: Path, path: Path) -> None:
    for attempt in range(_REPLACE_RETRIES):
        try:
            ...
            if os.name != "nt" or attempt + 1 >= _REPLACE_RETRIES:
                raise
            time.sleep(_REPLACE_PAUSE_SEC * (attempt + 1))
