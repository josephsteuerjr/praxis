...
class ThreeOutcomesNotOne(RunLabelBase):
    ...
    # ⚠ ПОРТ 17.09: причины здесь ЕДУТ ДОСЛОВНО, а не перефразируются. У ядра исход
    # переписывал расписку своими словами; издание печатает три собственные формулировки,
    # и две из них содержат подстроку «without a reply hand» — та лестница молча съела бы
    # диагностику обрыва потолком. Дословный перенос заодно снимает вопрос «что именно
    # сказала расписка»: исход её читает, а не заменяет.
    def test_a_turn_without_a_reply_hand_is_not_her_decision(self):
        why = "turn ended without a reply hand (not a declared silence)"
        self.assertTrue(self.close_with_reason(why))
        self.assertEqual(self.terminal().get("reason"), why)

    def test_a_ceiling_cut_is_not_mistaken_for_a_missing_hand(self):
        """Обрыв потолком — самый важный из трёх, и он НЕ должен схлопнуться в общий случай."""
        why = ("turn ended without a reply hand: model response cut by max_tokens "
               "(1200 chars, glm-5.3)")
        self.assertTrue(self.close_with_reason(why))
        self.assertEqual(self.terminal().get("reason"), why)
