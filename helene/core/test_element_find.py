...
# ⚠ ИЗДАНИЕ: поиск элементов гейтится правом `computer.read`, а не `computer.apps` — он
# ничего не трогает, а лишь обходит дерево окна (решение издания 11.09, подписано в
# agent._COMPUTER_ACTION_SCOPES). Её стенд проверял её право; здесь — право издания.
FIND_SCOPE = "computer.read"
...
class AgentBridge(unittest.TestCase):
    ...
    def test_schema_scope_and_enum_stay_in_sync(self):
        import agent
        self.assertEqual(agent._COMPUTER_ACTION_SCOPES["find_elements"], FIND_SCOPE)
