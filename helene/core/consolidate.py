...
EXTRACT_SYSTEM = (
    "You are the quiet memory analyst of the live agent. Input: journal entries "
    ...
    "Write all field values in RUSSIAN — this is the agent's memory, in its language."
)

PORTRAIT_SYSTEM = (
    "Before you is your file on one person. Compose a short, coherent portrait — "
...
SELF_DISTILL_SYSTEM = (
    "You are revising your compact operational self-model from evidence. Return a concise "
...
def _apply_person(name: str, upd: dict) -> bool:
    ...
    try:
        ...
        if woke:
            log.info("consolidate: новый разговор — разбужено %d спящих нитей у %s", woke, slug)
    except Exception:
