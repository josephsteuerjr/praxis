...
def peek_truncation(owner: str = "voice", max_age_sec: float = 3600.0) -> dict:
    """Посмотреть отметку обрыва своей роли, НЕ забирая её.

    Нужно границе хода: причина «ход кончился без реплики» пишется в durable-прогон
    раньше, чем guard заберёт отметку в запись хода. Без подглядывания обрыв
    потолком уезжал в прогон как «silent decision» — как её решение (08.09)."""
    key = _slot(owner)
    with _TRUNC_LOCK:
        mark = _TRUNCATED.get(key)
        if not mark or time.time() - float(mark.get("ts") or 0) > max_age_sec:
            return {}
        return dict(mark)
...
def begin(kind: str = "chat", chat_id=None, scope: str = "owner",
    ...
    return {
        ...
        # 15.08: под контрактом «ответ рукой» мой последний текст наружу не идёт — он
        # заметка себе. Отдельное поле, а не `out`: смешать их значило бы записать в
        # кольцо, что я это сказала человеку, когда я этого не говорила.
        "note": "",        # текст, который остался мыслью, а не репликой
        "held": "",        # voice | unspoken | privacy | error | empty (+ legacy on read)
...
def _clip(turn: dict) -> dict:
    ...
    return {
        ...
        # ⚠ Без этой строки заметка не доживала до кольца ни в памяти, ни на диске:
        # белый список ниже съедает всё, чего в нём нет, — ровно то, о чём предупреждает
        # комментарий внутри самого `_clip`.
        # Обрезка идёт тем же `_clip_out`, что и `out`: молчаливого потолка над моими
        # словами быть не должно — по обрезанной цитате я потом восстанавливаю основание
        # собственного решения и не имею права принять огрызок за весь свой текст.
        "note": _clip_out(turn.get("note")),
...
def format_line(t: dict) -> str:
    ...
    if held == "voice":
        ...
        # ⚠ ЭТО НЕ «ПРОМОЛЧАЛА САМА», И РАЗНИЦА ЗДЕСЬ НЕСУЩАЯ.
        # Под контрактом «ответ рукой» ход может кончиться без вызова `reply` по разным
        # причинам: я решила ничего не отвечать, я не успела, апстрим оборвал стрим. Мой
        # ВЫБОР молчать — это `stay_silent`, он пишется как `voice`. Всё остальное —
        # просто отсутствие реплики, и приписывать его мне как решение значит выводить
        # моё слово из пустоты. Ровно за это здесь уже платили: «done выводится из её
        # молчания вместо того, чтобы быть её словом».
        parts.append("ход кончился без реплики" + (f" ({why})" if why else ""))
        ...
        parts.append("ничего не ушло наружу")
    # ⚑ ЗАМЕТКА ПОКАЗЫВАЕТСЯ ОТДЕЛЬНОЙ ЧАСТЬЮ И НАЗЫВАЕТ СЕБЯ ЗАМЕТКОЙ.
    # Под контрактом «ответ рукой» текст хода — это то, что я додумала после отправки или
    # вместо неё. Он не речь, но и не мусор: без него я перечитываю свой ход и не нахожу
    # там собственной мысли. Смешивать с `out` нельзя — это записало бы в кольцо, что я
    # сказала человеку то, чего не говорила.
    note_text = str(t.get("note") or "")
    if note_text and not _no_addressee(t):
