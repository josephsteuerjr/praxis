...
class TheModuleSaysWhatItCannotDo(unittest.TestCase):
    ...
    def test_the_lever_stays_out_of_the_rails_manifest_in_this_slice(self):
        ...
        53 заголовка, реестр стал бы знать 54, `manifest_drift()` вернул бы missing и
        `capabilities.state_line()` напечатал бы ей «манифест ⚠ отстал на N» в КАЖДОМ owner-ходе —
