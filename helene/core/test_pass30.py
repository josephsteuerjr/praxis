...
class FsProbeImageTests(unittest.TestCase):
    ...
    def test_fs_read_of_an_image_tells_a_text_only_voice_it_sees_nothing(self):
        # 08.09: GLM-5.3 картинок не принимает; фраза «inspect the attached pixels»
        # толкала описывать экран по памяти (06.09, отозвано ею самой 07.09).
        import agent
        import workshop
        ...
        with mock.patch.object(workshop, "BASE", self.base), \
                ...
                mock.patch("hands.guard", return_value=None), \
                mock.patch("llm.accepts_images", return_value=False), \
                mock.patch("llm.can_see", return_value=False), \
                mock.patch("llm.role_model", return_value="glm-5.3"):
            # 09.09: гейт пикселей — llm.can_see (зрячая модель ИЛИ зрячая замена); текстовая
            # ветка — когда замены нет. С умолчанием glm → glm-5.3-flash сюда попадёт только
            # роль с явно отключённой заменой, поэтому can_see здесь глушится отдельно.
            seen = agent.tool_fs_read(str(image))
        self.assertIsInstance(seen, str, "без зрения image-блок не прикладывается")
        self.assertIn("glm-5.3", seen)
        self.assertIn("NO pixels", seen)
        self.assertIn("read_window", seen)
        self.assertIn("frame.jpg", seen)
...
class ObserveFileTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env.start()
        # Пиксели прикладываются только зрячей модели; text-only ветка — в
        # FsProbeImageTests.test_fs_read_of_an_image_tells_a_text_only_voice_it_sees_nothing.
