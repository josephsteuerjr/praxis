...
def capture(*, ctx, history, speaker, user_msg, tools,
    ...
    metrics = {
        ...
        # 08.09: сшивка тени с входом модели и строкой llm_calls — по тем же run_id и
        # frame_id, а не по восьми знакам имени файла (СТАТИСТИКА-КАДРА-08.09, п. 1).
        "run_id": (run.run_id if run is not None else ""),
        "frame_id": getattr(frame_trace.current(), "frame_id", "") or "",
