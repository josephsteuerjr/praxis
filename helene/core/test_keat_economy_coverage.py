...
class RendererCoverageTests(LiveTests):
    def check_coverage(self, *, filename='synthetic.md', mention=False,
        ...
        # Издание: байты досье — LF и на Windows (в движке это делает LF-шим дерева,
        # `sitecustomize`; нативный прогон стенда его не грузит).
        (people / filename).write_bytes(selected_body.encode('utf-8'))
        (root / 'soul').mkdir()
