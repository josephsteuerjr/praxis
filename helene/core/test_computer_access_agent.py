...
class AgentComputerAccessTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env.start()
        # Здесь проверяются руки зрячей модели; честный отказ text-only модели (GLM)
        # накрыт отдельно — test_pass30.FsProbeImageTests и test_observe_… ниже.
        self.sighted = mock.patch("llm.accepts_images", return_value=True)
        self.sighted.start()
    ...
    def test_powershell_scripts_with_cyrillic_get_a_bom_and_nothing_else_does(self):
        # 08.09: PowerShell 5.1 читает .ps1 без BOM как ANSI — кириллица ломала парсер,
        # форма Миры не поднялась, пока она сама не перезаписала файл с BOM.
        import agent
        ctx = agent.ChannelContext(chat_id="101", principal_id="101", is_dm=True, known=True, owner=True)
        token = agent._TURN_CHANNEL.set(ctx)
        try:
            ok = {"ok": True, "type": "result"}
            with mock.patch.object(agent, "_computer_allowed", return_value=True), \
                    ...
                    mock.patch("body_client.call", return_value=ok) as call:
                agent.tool_computer("write", path=r"C:\t\форма.ps1", content="Write-Host 'Привет'")
                agent.tool_computer("write", path=r"C:\t\plain.ps1", content="Write-Host 'hi'")
                agent.tool_computer("write", path=r"C:\t\заметка.txt", content="Привет")
                agent.tool_computer("write", path=r"C:\t\bom.ps1", content="\ufeffWrite-Host 'Привет'")
            bodies = [c.args[1]["content"] for c in call.call_args_list]
            self.assertEqual(bodies[0], "\ufeffWrite-Host 'Привет'", "кириллица в .ps1 → BOM")
            self.assertEqual(bodies[1], "Write-Host 'hi'", "ASCII не трогаем")
            self.assertEqual(bodies[2], "Привет", "не PowerShell — байт-в-байт")
            self.assertEqual(bodies[3], "\ufeffWrite-Host 'Привет'", "второй BOM не ставится")
        finally:
            agent._TURN_CHANNEL.reset(token)
    ...
    def test_computer_apps_grant_routes_typed_native_desktop_actions(self):
        ...
        try:
            ...
            with mock.patch("body_client.desktop_status", return_value=ok) as status, \
                    ...
                    mock.patch("body_client.desktop_window_read", return_value=ok) as window_read, \
                    mock.patch("body_client.format_window_read", return_value="native tree") as fmt_read, \
