...
_ACT = (
    ...
    # 13.08: список действий отставал от того, что она реально обещает. Живая реплика
    # «Сейчас найду настоящий checkout через serverd и открою задачу» не распозналась —
    # ни «найду», ни «открою» в списке не было. Добавлены глаголы работы с материалом:
    # именно они звучат, когда ей кидают файл и просят прочитать, свести и написать.
    r"найду|открою|прочитаю|прочту|поищу|изучу|сверю|сравню|отредактирую|реализую|"
    r"ищу|читаю|открываю|сверяю|редактирую|изучаю)"
)
...
_PROMISE_PATTERNS = (
    ...
    # ⚠ 08.09: без `\b` после глагола «сейчас вернуЛА 0» читалось как «верну» —
    # прошедшее время становилось обещанием, и Мира в Hélène получила ложное
    # пробуждение с гистом из середины чужой фразы. Граница слова обязательна.
    ...
    # 13.08: ПЕРФОРМАТИВ БЕЗ ВРЕМЕНИ — дыра, найденная на живой личке 12.08. Все паттерны
    # выше требуют временной привязки («сейчас», «через N минут», «к 18:00»), а её самая
    # частая форма обещания её не имеет: «Исправляюсь: напишу туда, не в канал»,
    # «Исправляю.», «Отправляю». Это глагол-действие в настоящем времени, которым
    # объявляется САМ АКТ, — и ровно после него ход закрывался, а человек видел обещание.
    #
    # Узко намеренно: только начало предложения или строки, только замкнутый список
    # перформативов, ничего условного. «Если хочешь, напишу подробнее» под него не
    # попадает — там нет ни одного из этих глаголов в начале клаузы.
    re.compile(r"(?im)^\s*(?:исправляю(?:сь)?|поправляю(?:сь)?|отправляю|высылаю|"
               r"переключаю(?:сь)?|запускаю|исправлюсь)\b"),
)
...
def detect(text: str) -> str | None:
    ...
    for pattern in _PROMISE_PATTERNS:
        ...
        if found:
            # Гист режется по границам слов: срез «-40 знаков» посреди слова давал
            # «реднее окно» вместо «переднее окно» (пробуждение 08.09).
