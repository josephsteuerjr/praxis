...
# порт: shell=True на винде = cmd.exe, одинарные кавычки shlex он не понимает,
# а путь питона содержит пробел — на win32 кавычим двойными (no-op на POSIX).
_PY = (f'"{__import__("sys").executable}"' if __import__("sys").platform == "win32"
       else shlex.quote(__import__("sys").executable))
import tempfile
import unittest
