...
from keat_candidate import RECEIPT_TRANSFORM, RENDER_TRANSFORM, _digest, is_render_receipt
...
def _scheduled_wake_payload(task, occurrence):
    ...
    if task.get('kind') == 'message':
        ...
        # Издание: сид без рода — та же формула, что у живого `agent.wake_turn`.
        seed = (f"Твоя просьба разбудить себя вот с чем: {goal}" if goal else
...
class Bound:
    ...
    # Издание (26.09): преобразование каждого сообщения проекции — RENDER_TRANSFORM или
    # RECEIPT_TRANSFORM (расписка ленты-вызовов, без происхождения). Длина = длине refs.
    transforms: object = None


def _per_message(groups, rendered):
    """Группы происхождения по записям ленты -> по сообщениям кадра (издание, 26.09).

    Сборщик рисует её прошлую реплику парой «вызов руки reply + расписка»; расписка —
    отдельное сообщение без происхождения (`keat_candidate.render_receipt`). Остальные
    сообщения получают свои группы по порядку. Лишняя или недостающая группа — отказ:
    проекция обязана покрыть кадр целиком, а не его хвост.
    """
    refs, transforms, pending = [], [], list(groups)
    for index in range(len(rendered)):
        if is_render_receipt(rendered, index):
            refs.append([])
            transforms.append(RECEIPT_TRANSFORM)
            continue
        _require(bool(pending), 'projection shorter than rendered tape')
        refs.append(pending.pop(0))
        transforms.append(RENDER_TRANSFORM)
    _require(not pending, 'projection longer than rendered tape')
...
def bind_turn(state, messages, fallback_current=None, candidate_messages=None):
    ...
    try:
        if state is not None:
            ...
            receipts = sum(1 for index in range(len(projection))
                           if is_render_receipt(projection, index))
            groups = (state.projection_groups if state.projection_groups is not None
                      else [[ref] for ref in _references(state, len(projection) - 1 - receipts)])
            refs, transforms = _per_message(groups, projection)
            state.historical_count = len(refs) - 1
            bound = Bound(state, messages, _digest(projection), refs,
                          copy.deepcopy(fallback_current), transforms=transforms)
    except Exception:
        if state is not None:
...
def capture_appended(messages, additions):
    ...
    try:
        ...
        bound.transforms.extend([RENDER_TRANSFORM] * len(refs))
...
def valid_served_receipt(receipt, *, system, messages, tools, run_id, call_id):
    ...
    try:
        ...
        _require(messages is bound.provider_messages and
                 ...
                 _digest([bound.refs, bound.transforms]) == refs_digest,
                 'changed selection projection')
        return True
    except Exception:
        return False
    ...
    try:
        ...
        with state.ledger.guard():
            state.ledger.project(
                ...
                transforms=list(bound.transforms),
...
def _receipt(state, binding, system, messages, tools):
    ...
    bound.served_selection = (copy.deepcopy(receipt),
                              _digest([system, messages, tools]),
                              _digest([bound.refs, bound.transforms]))
    return receipt
...
def bind_resume(model_input, run_id, call_id):
    ...
    try:
        ...
        transforms = [entry['transform'] for entry in source['projection']]
