...
class LeanProjectIsVisibleToTheMatrix(unittest.TestCase):
    ...
    def test_the_projects_own_wrapper_wins(self):
        ...
        # порт: shlex.quote на POSIX для простого пути — тождество, на винде
        # обязан взять путь с бэкслешами в кавычки для sh
        self.assertEqual(row["command"], "sh " + shlex.quote(str(wrapper)))
