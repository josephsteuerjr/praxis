...
class TestOwnedTruncation(TurnsBase):
    ...
    def test_peek_lets_the_turn_boundary_name_the_cut_without_taking_it(self):
        # 08.09: граница хода пишет причину «без реплики» в durable-прогон РАНЬШЕ, чем
        # guard заберёт отметку. Подглядывание не снимает её и не мешает guard.
        self.assertEqual(turns.peek_truncation(), {})
        turns.note_truncated(model="glm-5.3", chars=0)
        seen = turns.peek_truncation()
        self.assertEqual((seen.get("model"), seen.get("chars")), ("glm-5.3", 0))
        self.assertEqual(turns.peek_truncation(owner="evaluator"), {},
                         "чужой слот не виден")
        claimed = turns.take_truncation()
        self.assertEqual(claimed.get("model"), "glm-5.3", "guard забирает после подглядывания")
        self.assertEqual(turns.peek_truncation(), {}, "забранное больше не видно")
