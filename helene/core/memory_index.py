...
def build() -> dict:
    ...
    if owner is None:
        # ⚠ Издание (ревью 26.09, W3 S10): «замок держит другой сборщик» отличимо от
        # «пересборка упала» — поток ухода за recall называл одно другим.
...
def _build_owned() -> dict:
    ...
    if not _embeddings_on():
        out = {"model": _index_model(), "files": {}, "records": [], "fts": fts}
        if error:
            out["fts_error"] = error
        return out
    records: list[dict] = []
    ...
    return dict(index, fts_error=error) if error else index
