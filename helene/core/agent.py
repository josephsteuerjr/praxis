...
_COMPUTER_ACTION_SCOPES = {
    ...
    # 08.09: чтение окна через UI Automation. Тело и body_client умели это давно
...
def tool_computer(action: str, path: str = "", caption: str = "", command: str = "",
    ...
    if action in {"write", "replace"}:
        ...
        if action == "write":
            ...
            if (str(path).lower().endswith((".ps1", ".psm1", ".psd1")) and body
                    and not body.startswith("\ufeff") and not body.isascii()):
                # PowerShell 5.1 читает .ps1 без BOM как ANSI: кириллица в строках
                # ломает парсер (живой прогон Миры 08.09 — форма не поднялась, пока
                # она сама не перезаписала файл с BOM). Тело пишет UTF-8 байты строки,
                # поэтому BOM ставится здесь, символом.
                body = "\ufeff" + body
...
COMPUTER_TOOL = {
    ...
    "description": (
        ...
        "shape=tree|flat, text_contains to filter, max_nodes/max_depth to bound; the receipt names every limit and "
        "what stayed unread. Prefer the "
        "typed type_text/hotkey/key/move/click/scroll actions; input accepts one ordered mixed events batch. After "
        "activating a window do not assume the caret is free: many controls (WinForms TextBox) select all text when "
        "focus arrives with the window, and type_text would REPLACE it — click once into the field or press End "
        "before typing; Escape keeps the selection, and a second click right after the first is a double-click that "
        "selects a word. For scroll, "
        ...
        "screenshot→act→observe loop ONLY when the voice model accepts images — a text-only model (GLM-5.3) gets a "
        "reference and no pixels, so it must not describe the screen from observe and should read_window instead; "
        "observe with path=<file> attaches an IMAGE FILE from the computer's disk "
    ...
    "input_schema": _obj({
        ...
        "shape": {"type": "string", "enum": ["tree", "flat"],
                  "description": "read_window: tree keeps parent links; flat lists only matching nodes"},
...
# ──────────────────────────────────────────────── указатель рук (КЕАТ, порт 18.09)
#
# ⚠ ПОРТ В ИЗДАНИЕ. Здесь тот же механизм, что в ядре, с одной разницей: указатель
# печатается ПО-АНГЛИЙСКИ (`tool_text_en.pointer_*`). В ядре его текст русский, и
# включение указателей там выбрасывает английские описания 86 рук из 95 — ровно то,
# ради чего 15.09 положен `tool_text_en.py`. У издания обе вещи обязаны работать
# вместе: кадр дешевле И схемы по-английски.
...
def _hand_purpose(tool: dict) -> str:
    """Строка указателя про руку. Английская проекция — той же накладкой, что схемы.

    ⚠ Без неё «Кадр — да» отменяет «Англ — да»: указатель заменяет сотню схем своими
    строками, и русский текст выбросил бы английские описания 86 рук из 95.
    """
    name = str(tool.get("name") or "")
    known = HAND_PURPOSE.get(name)
    if known:
        return tool_text_en.pointer_purpose(name, known)
...
def hands_pointer_text(catalog: list) -> str:
    ...
    Русские литералы здесь — источник правды и её собственная документация; модели
    уезжает английская проекция (`tool_text_en.pointer_*`), у которой записан отпечаток
    каждого оригинала. Правят русскую строку — прибор накладки краснеет её именем.
    """
    ...
    natives_ru = ("Со схемой в кадре — родные, которыми хожу чаще всего: {names}"
                  "{provider}.")
    provider_ru = " и провайдерский web_search" if provider else ""
    rest_ru = ("Остальные — те же мои руки, только их схема не едет в каждый кадр. Зову их через "
    ...
    columns_ru = "Имя(аргументы) — назначение:"
    lines = [tool_text_en.pointer_head("title", title_ru),
             tool_text_en.pointer_head("natives", natives_ru)
             .replace("{names}", ", ".join(sorted(natives)))
             .replace("{provider}", tool_text_en.pointer_head("provider", provider_ru)
                      if provider else ""),
             tool_text_en.pointer_head("rest", rest_ru),
             tool_text_en.pointer_head("columns", columns_ru)]
    placed: set[str] = set()
    for group, names in HAND_GROUPS:
        ...
        if rows:
            lines.append(f"— {tool_text_en.pointer_group(group)}: " + "; ".join(rows) + ".")
    rest = [n for n in sorted(by_name) if n not in placed and n not in NATIVE_HAND_NAMES]
    if rest:
        lines.append(f"— {tool_text_en.pointer_head('other', 'прочее')}: " + "; ".join(
...
# 13.09, слово Егора: показывать меньше сырых сообщений, но видеть НОРМАЛЬНУЮ свёртку
# того, что было. При 4 000 знаках в кадр влезали три последних компакта из 23 тыс.
...
# Продукт с собственной границей доставки (Hélène: `agent.deliver_unspoken`) поднимает
# этот флаг при старте. Тогда ход, закрывшийся текстом без руки `reply`, не терминализуется
# здесь как молчание — слово доставит граница и сама положит расписки. На сервере (mtproto)
# флаг опущен: там текст без руки и есть молчание, поведение прежнее байт-в-байт.
...
def _delivery_evidence(run_id: str) -> dict:
    ...
    for row in _runs().iter_events(run_id):
        ...
        if (kind == "tool_started" and row.get("tool") == "telegram.deliver"
            ...
            skipped = True
            # Причина молчания — из расписки, а не из умолчания редьюсера: «ход кончился
            # без реплики» и «оборвано потолком» не равны «решила промолчать» (08.09).
            skipped_reason = str(row.get("reason") or "")
...
def offered_tools_for(ctx: "ChannelContext") -> list:
    ...
    При указателях (`tool_pointers_enabled`) — родные плюс `describe` и `call`;
    полный список того, что модель МОЖЕТ позвать, остаётся в `catalog_tools_for`,
    и он же едет в кадр строками «имя(аргументы) — назначение».
    ...
    # Схемы `describe`/`call` — через ту же английскую накладку, что и остальные:
    # иначе две родные руки говорили бы по-русски посреди английского набора.
    return body + tool_text_en.apply([DESCRIBE_TOOL, CALL_TOOL]) + closer
    ...
    while max_iters is None or iteration < max(0, int(max_iters)):
        ...
        for b in resp.blocks:
            ...
            if b["type"] != "tool_use":
                continue
            assistant_blocks.append(b)
            # `call(name, args_json)` переписывается во внутреннюю руку ЗДЕСЬ — до
            # durable-записи, побочных эффектов и потолка времени: воронка одна на всех,
            # и в ленте модели остаётся её собственный вызов `call`.
            ...
            if b.get("name") not in offered_names and not dispatched:
                # Имя из каталога ЭТОГО хода, позванное напрямую (модель запомнила его
                # из указателя): власть та же, что у `call`, — каталог хода, а не
                # TOOL_IMPL целиком.
...
def voice_turn_envelope(chat_id: str | int | None, convo_text: str, speaker: str | None = None,
    ...
    if work_loop.reply_hand_enabled():
        ...
        if chose_silence:
            ...
            if durable_id and BOUNDARY_DELIVERS_UNSPOKEN and str(reply or "").strip():
                # Слово написано текстом, и продукт (Hélène) доставит его на границе
                # окна сам: прогон остаётся running, расписки started → text_accepted →
                # finalize положит граница (`runner._close_run`). Раньше здесь ставился
                # silent-терминал, и граница била в уже закрытый прогон: карточка не
                # видела ни доставки, ни самого слова (08.09).
                log.debug("прогон %s оставлен границе продукта: слово уйдёт ею", durable_id)
            elif durable_id:
                # Обрыв потолком max_tokens до первого слова — четвёртый исход, и он
                # не её решение: пробуждение 08.09 (8192 токенов размышления, ноль
                # текста, обрубленный tool_use) легло в прогон как «silent decision».
                ...
                if work_loop.finished(durable_id or ""):
                    ...
                    why = ("turn ended without a reply hand: model response cut by "
                           ...
                           f"{cut.get('model') or 'model'})")
                else:
                    why = "turn ended without a reply hand (not a declared silence)"
                run_delivery_completed(durable_id, silent=True, silent_reason=why)
        else:
