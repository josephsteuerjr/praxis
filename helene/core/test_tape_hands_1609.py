...
class TheTapeNeverShowsAHandThatIsNotGiven(unittest.TestCase):
    """⚠ ПРАВКА ИЗДАНИЯ, КОТОРОЙ В ЯДРЕ НЕТ.

    У ядра руки `reply`/`end_turn` подняты всегда, и одного рычага довольно. Здесь набор
    режется по месту — под опущенным `PRAXIS_CHAT_REPLY_HAND` и на ходе без комнаты руки
    `reply` в наборе нет вовсе. Лента, показывающая вызов несуществующей руки, учит
    модель звать то, чего ей не дали: вместо прозы приезжает выдуманный вызов, то есть тот
    же дефект с другой стороны. Поэтому решает не рычаг в одиночку, а тот, кто набор уже
    собрал.
    """

    def test_lever_alone_still_turns_it_on(self):
        with _on():
            out = frame_layout.tape(ROWS)
        self.assertTrue(any(b.get("type") == "tool_use"
                            for m in out if isinstance(m.get("content"), list)
                            for b in m["content"] if isinstance(b, dict)))

    def test_an_explicit_no_beats_the_lever(self):
        with _on():
            out = frame_layout.tape(ROWS, hands=False)
        self.assertEqual(out, frame_layout.tape(ROWS, hands=False))
        self.assertFalse(any(b.get("type") == "tool_use"
                             for m in out if isinstance(m.get("content"), list)
                             for b in m["content"] if isinstance(b, dict)),
                         "лента показала вызов руки, которой на этом ходе не дают")

    def test_an_explicit_yes_beats_a_lowered_lever(self):
        """Признак снаружи — ответ, а не подсказка: рычаг его не перебивает."""
        out = frame_layout.tape(ROWS, hands=True)
        self.assertTrue(any(b.get("type") == "tool_use"
                            for m in out if isinstance(m.get("content"), list)
                            for b in m["content"] if isinstance(b, dict)))

    def test_the_predicate_repeats_the_cut_word_for_word(self):
        """`agent.reply_hand_offered` обязан повторять вырезы набора, а не гадать."""
        import agent  # локально: модуль тяжёлый, а нужен одному классу
        ctx = agent.ChannelContext.from_legacy("-100500", is_dm=False, owner=False)
        no_room = agent.ChannelContext.from_legacy(None, is_dm=True, owner=True)
        with mock.patch.object(agent.work_loop, "reply_hand_enabled", lambda: True):
            self.assertTrue(agent.reply_hand_offered(ctx))
            self.assertFalse(agent.reply_hand_offered(no_room),
                             "ход без комнаты руку `reply` не получает — лента не вправе её показать")
            self.assertFalse(agent.reply_hand_offered(ctx, no_tools=True))
            self.assertFalse(agent.reply_hand_offered(ctx, tools_override=[]))
            self.assertTrue(agent.reply_hand_offered(ctx, tools_override=[{"name": "reply"}]))
        with mock.patch.object(agent.work_loop, "reply_hand_enabled", lambda: False):
            self.assertFalse(agent.reply_hand_offered(ctx),
                             "рычаг рук опущен — руки нет, и образца ей в ленте не место")
