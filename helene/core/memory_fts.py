...
_CLAIM_FDS: dict[str, int | None] = {}
_CLAIM_BUILDER_FDS: dict[str, int] = {}
# ⚠ WINDOWS (издание Hélène). Её механизм заявок стоит на POSIX: `flock` на открытом
# описании файла и `rename`/`unlink` заявки при ОТКРЫТОМ дескрипторе. На Windows `flock`
# нет вовсе, а `os.open` не даёт FILE_SHARE_DELETE — открытый файл нельзя ни
# переименовать, ни удалить. Взятый как есть, захват падал бы OSError → «заявка есть,
# захватить не вышло» → `memory_index.build` не пересобирал бы индекс НИКОГДА (pending без
# claim — пропуск), а явный recall после сбоя базы молчал бы до конца времён.
# Поэтому на Windows единственный живой замок — замок сборщика (`msvcrt.locking`, байт 0
# стабильного `recall_builder.lock`), а заявка — просто данные. Все, кто трогает заявки
# (`claim_refresh_request`, `memory_index._build_owned`), держат замок сборщика; значит,
# чужая заявка, увиденная под нашим замком, осиротела — её сборщик умер, не отпустив
# (окно издания закрывают посреди пересборки, это обычный случай, а не авария).
_WINDOWS = os.name == "nt"
_WINDOWS_LOCK_WAIT_SECONDS = 600.0
...
def _claim_lock(fd: int, *, nonblocking: bool = False) -> bool:
    ...
    if _WINDOWS:
        import msvcrt
        deadline = time.monotonic() + _WINDOWS_LOCK_WAIT_SECONDS
        while True:
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise TimeoutError("recall builder lock wait timed out")
                time.sleep(0.05)
    try:
        import fcntl
...
def release_builder_lock(fd: int) -> None:
    ...
    if _WINDOWS:
        # Windows снимает замок закрытого хэндла «когда найдёт ресурсы» — снимаем явно.
...
def _recover_stale_refresh_claims(*, memory_dir: Path) -> None:
    ...
    if _WINDOWS:
        # Зовут только под замком сборщика (см. `_WINDOWS` вверху): чужая заявка здесь
        # осиротела, аренду ждать нечего — иначе закрытое посреди пересборки окно
        # останавливало бы обслуживание памяти на шесть часов. rename на Windows —
        # атомарная проверка «новой заявки ещё нет» (FileExistsError), как link() у неё.
        for claim in request.parent.glob("recall_refresh.claim.*.json"):
            if str(claim) in _CLAIM_FDS:
                continue
            try:
                os.rename(claim, request)
...
def _claim_refresh_request_locked(*, memory_dir: Path) -> Path | None:
    ...
    if _WINDOWS:
        # Без открытого дескриптора: rename открытого файла на Windows запрещён, а
        # владение держит замок сборщика у вызывающего.
        try:
        ...
        data.update({"claimed_at": now, "lease_expires_at": now + _REFRESH_CLAIM_LEASE_SECONDS,
                     "owner": {"pid": os.getpid()}})
        with contextlib.suppress(OSError):
            with open(claim, "w", encoding="utf-8") as out:
                out.write(json.dumps(data, ensure_ascii=False))
                out.flush()
                os.fsync(out.fileno())
        _CLAIM_FDS[str(claim)] = None
        return claim
    fd = None
    try:
...
def restore_refresh_request(*, memory_dir: Path, claim: Path) -> None:
    ...
    request = _refresh_request_path(Path(memory_dir))
    if _WINDOWS:
        # rename вместо link: жёстких ссылок может не быть (не-NTFS), а атомарный отказ
        # при существующей новой заявке rename на Windows даёт сам.
        try:
            try:
                os.rename(claim, request)
            except FileExistsError:
