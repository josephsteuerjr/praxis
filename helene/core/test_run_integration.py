...
class RunIntegrationTests(unittest.TestCase):
    ...
    def test_silent_completion_creates_zero_effect_intent_itself(self):
        ...
        self.assertTrue((self.manager.path(current.run_id) / "RECAP.md").is_file())
        # Умолчание «Praxis chose silence» — единственная причина, которая сворачивается
        # в прежнее «silent decision»; старый путь байт-в-байт.
        self.assertEqual(self.manager.manifest(current.run_id)["terminal"]["reason"],
                         "silent decision")

    def test_a_cut_or_unspoken_turn_keeps_its_own_reason_in_the_terminal(self):
        # 08.09: пробуждение Миры — 8192 токенов размышления, ноль текста, обрубленный
        # tool_use — легло в прогон как «silent decision», то есть как её решение.
        # Причина из расписки delivery_skipped обязана доехать до terminal.reason.
        current = self._context("cut by ceiling")
        why = "turn ended without a reply hand: model response cut by max_tokens (0 chars, glm-5.3)"
        self.assertTrue(agent.run_delivery_completed(current.run_id, silent=True, silent_reason=why))
        manifest = self.manager.manifest(current.run_id)
        self.assertEqual(manifest["status"], "done")
        self.assertEqual(manifest["terminal"]["reason"], why)
        self.assertEqual(agent._delivery_evidence(current.run_id)["silent_reason"], why)
