# Praxis — the window onto a core that lives somewhere else

The second application. Where [`desk/`](../desk) is the whole of Hélène — the window, the
channel, the runner and the installer around a core on this machine — Praxis is only the
...
the settings screen of Praxis scolding a foreign harness for the absence of things a
...
| [`desk/remote`](../desk/remote) | **Praxis.** The agent lives on a server. One card says so; the rest of that screen is the window's own business — names, phone, the server address, updates. It also asks for that address on first run, because this variant has no installer by design. |
...
frame must not know the local cards, Praxis must not import them, and whatever differs
between the two the frame asks for instead of deciding. The Praxis bundle is 158 kB against
Hélène's 200 kB — the local agent's cards are not merely hidden there, they are absent.
...
channel, the same package. Praxis is an application built from `desk/`, not a separate
