...
class TestStatusIsNotCanon(unittest.TestCase):
    ...
    def test_stale_live_head_is_gone(self):
        """Предмет — свежесть ссылки на live-код, а не название строки.

        ⚠ 07.08.2026 тест покраснел не от порчи файла, а от ЕЁ САМОПРАВКИ: предложение
        `effa9c51` переименовало «Последний live-код прода» в «Live ref для этого снимка»
        и добавило прямую оговорку, что файл — датированная точка чтения, а не
        самособирающийся источник истины. Она не подставила свежий SHA в старую форму —
        она переопределила ЖАНР файла, чтобы он перестал притворяться каноном.

        Жёсткая привязка к одному заголовку делала тест сторожем ФОРМУЛИРОВКИ, а сторожить
        он обязан ссылку. Принимаются обе формы; растёт список — растёт и он, но предмет
        остаётся один: в файле есть ссылка на живой код и она не июльская.
        """
        head = re.search(
            r"\*\*(?:Последний live-код прода|Live ref для этого снимка):\*\*\s*`([0-9a-f]{7,40})`",
            self.text)
        self.assertIsNotNone(head, "в STATUS.md не осталось ссылки на живой код прода")
        self.assertNotEqual(head.group(1), "e0150fe",
                            "это был live-код от 19.07; прод ушёл на 29 коммитов вперёд")
        self.assertNotEqual(head.group(1), "fb09430b",
                            "это был live-код от 01.08; он уже отставал, когда его называли")
